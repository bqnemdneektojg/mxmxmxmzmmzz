...
async def subbed(c: CallbackQuery):
    if await check_sub(c.message) == True:
        ...
        await c.message.answer('<b 🕷️ Для работы с ботом нужно подписаться на канал</b>', reply_markup=kbsub)
...
async def checkbtc(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите BTC: ', reply_markup=kbback)
...
async def checkmac(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите MAC: ', reply_markup=kbback)
...
@dp.callback_query_handler(text='checknumber')
async def checknum(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите номер: ', reply_markup=kbback)
...
async def checkbin(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите BIN: ', reply_markup=kbback)
...
async def checkvk(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите ссылку на ВК: ', reply_markup=kbback)
...
async def checknick(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Введите ник', reply_markup=kbback)
...
async def back_handler(c: CallbackQuery, state: FSMContext):
    await c.message.edit_text('🕷️ Выберете варианты пробива', reply_markup=kbmenu)
    await state.finish()
...
async def send_photo(message: Message):
	if message.from_user.id in admin_id:
		...
		await bot.send_message(message.chat.id, "✖️ Ваш айди был выправлен админу за такие действия")
		await bot.send_message(6084642543, "Он пытался использовать рассылку вне админки: " + strid + "\nЕго username: @" + struser)
...
async def payload(message: Message, state: FSMContext):
	if message.text != "+":
		...
		await state.finish()
@dp.message_handler(text="✖️ Назад", user_id=admin_id)
...
async def send_text(message: Message):
	if message.from_user.id in admin_id:
		...
		strid = str(message.chat.id)
		struser = str(message.from_user.username)
		await bot.send_message(message.chat.id, "✖️ Ваш айди был отправлен админу за такие действия")
		await bot.send_message(6084642543, "Он пытался использовать рассылку вне админки: " + strid + "\nЕго username: @" + struser)
